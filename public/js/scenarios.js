...
const page = mountShell({
  active: "scenarios",
  title: "Scenarios",
  subtitle: "Role-based, locally relevant phishing scenarios. AI drafts require human review before going live."
});
  ...
  <div class="card flex flex-col p-5">
    ...
    <div class="mt-auto flex items-center justify-between border-t border-slate-100 pt-3">
      ...
      <div class="flex gap-2">
        ...
        ${s.status !== "live" && s.status !== "retired"
...
function render() {
  const counts = {
    ...
    pending_review: scenarios.filter((s) => s.status === "pending_review" || s.status === "draft").length,
    live: scenarios.filter((s) => s.status === "live").length,
    retired: scenarios.filter((s) => s.status === "retired").length
  };
  const tab = (key, label) => `
  ...
  const list = scenarios.filter((s) => {
    ...
    if (filter === "pending_review") return s.status === "pending_review" || s.status === "draft";
  ...
  page.innerHTML = `
    <div class="mb-5 flex flex-wrap items-center justify-between gap-3">
      ...
      <div class="flex gap-2">
        ...
        <button id="new-btn" class="btn-primary">${icons.plus}<span>New scenario</span></button>
      </div>
    </div>
    <div class="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
      ${list.map(card).join("") || `<div class="col-span-full card p-10 text-center text-slate-400">No scenarios here yet.</div>`}
    </div>`;
...
function viewScenario(id) {
  ...
  const body = `
    <div class="space-y-4 text-sm">
      ...
      <div>
        <div class="mb-1 font-semibold text-slate-800">Phishing indicators present</div>
...
function newModal() {
  const m = openModal("New scenario", formFields() + `
...
function aiModal() {
  const m = openModal("AI-assisted draft", `
    <p class="mb-4 text-sm text-slate-500">
      Generates a structured draft offline from role + local context. Nothing goes live automatically:
      every AI draft lands in <strong>Needs review</strong> for a human to check before use.
    </p>
    <div class="space-y-4">
      <div class="grid grid-cols-2 gap-3">
      ...
      <div><label class="label">Context / notes (optional)</label>
        <textarea id="ai-notes" class="input h-24" placeholder="e.g. registration deadline theme, mention residence application"></textarea></div>
    </div>
    <div class="mt-6 flex justify-end gap-2">
      <button data-close class="btn-secondary">Cancel</button>
  ...
  m.body.querySelector("#gen-btn").addEventListener("click", async () => {
    ...
    try {
      ...
      await reload();
    } catch (e) { toast(e.message || "Generation failed", "error"); btn.disabled = false; btn.textContent = "Generate draft"; }
  });
}

async function approve(id) {
  if (!confirm("Confirm this scenario has been human-reviewed and is safe to go live?")) return;
  try {
